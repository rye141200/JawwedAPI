...
public class MushafServices(IGenericRepositoryMapped<Line, LineResponse> lineRepositoryMapped,IGenericRepositoryMapped<Verse,VerseResponse> verseRepositoryMapped, IGenericRepositoryMapped<Chapter,ChapterResponse> chapterRepositoryMapped) : IMushafServices
{
    ...
    public async Task<VerseResponse> GetVerseByChapterNumberAndVerseNumber(int chapterNumber, int verseNumber)
    {
        //! 1) check if the data is correct
        ...
        //! 2) Get the verse
        VerseResponse verse = await verseRepositoryMapped.FindOneMapped((verse) => verse.ChapterID == chapterNumber && verse.VerseNumber == verseNumber)
        ?? throw new GlobalErrorThrower(
                statusCode: 400,
    ...
    public async Task<ChapterResponse> GetVersesByChapterNumber(int chapterNumber){
        ...
        //! 2) Get the verses
        return await chapterRepositoryMapped.FindOneMappedPopulated(chapter => chapter.ChapterID == chapterNumber, chapter => chapter.Verses!) ?? throw new GlobalErrorThrower(
                statusCode: 400,
    ...
    public async Task<ChapterResponse> GetChapterMetaData(int chapterNumber)
    {
        ...
        //! 2) Get the chapter
        ChapterResponse chapter = await chapterRepositoryMapped.FindOneMapped(chapter => chapter.ChapterID == chapterNumber)
        ?? throw new GlobalErrorThrower(
                statusCode: 400,
                title: "Chapter not found",
                detail: $"The chapter with chapter number ${chapterNumber} was not found."
