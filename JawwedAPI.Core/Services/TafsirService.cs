...
public class TafsirService(IGenericRepository<Tafsir> TafsirRepository, IGenericRepository<Mofasir> MofasirRepository) : ITafsirService
{
    ...
    public async Task<List<TafsirResponse>> GetChapterTafsir(string? mofasirID, string? Chapter, string? language)
    {
        ...
        if (mofasirID == null || Chapter == null) throw new GlobalErrorThrower(400, "Data is missing", $"The provided data is missing {(mofasirID == null ? "mofasir ID" : "chapter number")}");
    ...
    {
        //! 1) check if mofasirName, Chapter not null 
        if (mofasirID == null || ChapterVerseID == null) throw new GlobalErrorThrower(400, "Data is missing", $"The provided data is missing {(mofasirID == null ? "mofasir ID" : "chapter or verse number")}");
        ...
        Tafsir? tafsir = await TafsirRepository.FindOneAndPopulateAsync(
            t => t.ChapterVerseID == ChapterVerseID &&
    ...
    private async Task ValidateMofasirAsync(string mofasirID, string language)
    {
        ...
        else
        {
            ...
            mofasir = await MofasirRepository.FindOne(
                m => m.AuthorNameArabic == mofasirID || m.AuthorNameEnglish == mofasirID
            );
        }
        ...
        {
            var authorName = language == "العربية" ? mofasir.AuthorNameArabic : mofasir.AuthorNameEnglish;
